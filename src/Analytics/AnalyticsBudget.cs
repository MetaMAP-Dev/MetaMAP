...
/// <summary>
/// Machine-specific send ledger for <see cref="Analytics"/>: a calendar-day record of what has
/// already been reported, plus a running monthly event count kept for observability.
/// </summary>
/// <remarks>
/// Ported from Eddy3D's <c>GUI.Analytics.AnalyticsBudget</c> (Eddy3D-Dev/Eddy3D). That class
/// enforced a monthly cap to protect Umami Cloud's 100k-events/month free tier; MetaMAP reports
/// to a self-hosted instance with no such billing ceiling, so no cap is enforced here either.
/// <see cref="TryClaimDaily"/> is the mechanism that still matters: a key (a ribbon tab, the
/// startup hit, a run) is sent at most once per machine per UTC day, so a Views figure is a
/// count of machine-days rather than how often someone reopens a definition — that has nothing
/// to do with billing.
/// State lives next to the plugin's own app-data folder (<c>%APPDATA%/MetaMAP</c>,
/// <c>~/Library/Application Support/MetaMAP</c> on macOS under net8.0). If the file cannot be
/// read or written the ledger degrades to an in-memory one for the current process.
/// </remarks>
public static class AnalyticsBudget
{
    ...
    private static readonly string DefaultStatePath = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "MetaMAP", "analytics.json");
    ...
    private static readonly HashSet<string> _sentToday = new(StringComparer.Ordinal);
    ...
    /// succeeds (no monthly cap on the self-hosted instance); the bool return is kept so call
    /// sites that check it don't need to change.
    /// </summary>
    ...
    /// </summary>
    /// <param name="key">What is being reported — <c>startup</c>, <c>tab/building</c>,
    /// <c>run/tab/metamap/fetch</c>. Case-sensitive.</param>
    /// <param name="cost">Events the payload will cost.</param>
    /// <returns><c>true</c> for the first caller today with this key; <c>false</c> means already
    /// sent today, and the caller drops the payload.</returns>
