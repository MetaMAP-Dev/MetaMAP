...
namespace MetaMap.Analytics;

/// <summary>
/// Privacy-first usage analytics via Umami — ported from Eddy3D's <c>GUI.Analytics.Analytics</c>
/// (Eddy3D-Dev/Eddy3D). Answers ONE question: <b>which ribbon tab do people use</b> — Building,
/// Terrain, Fetch — not which component, and not how often they reopen a file.
/// </summary>
/// <remarks>
/// <para><b>What is sent, and what a number in the dashboard means.</b> Everything is reported
/// at most once per machine per UTC day (<see cref="AnalyticsBudget.TryClaimDaily"/>):</para>
/// <list type="bullet">
...
/// the sink.</item>
/// <item><c>/tab/&lt;tab&gt;/&lt;action&gt;</c> — an event the first time each day an action is
/// actually run on that tab (e.g. a successful OSM fetch). Opening a definition is not using
/// it; running one is.</item>
/// </list>
/// <para><b>Umami bills per hit AND per stored data property</b> ("Each website hit counts as
/// one event. If you save event data, each data property stored counts as one event." —
/// docs.umami.is/docs/cloud/faq). Two consequences that must not be undone:</para>
/// <list type="bullet">
/// <item>The visitor hash rides in the <b>User-Agent</b>, not in <c>data</c> — free, and one of
/// the three inputs to Umami's session id.</item>
/// <item>Low-cardinality dimensions (tab, action) belong in the <b>url path</b>, not <c>data</c>.</item>
/// </list>
/// <para>Opt-out: set the environment variable <c>METAMAP_ANALYTICS=0</c> before starting Rhino
/// (<see cref="OptOutVariable"/>).</para>
/// </remarks>
public static class Analytics
{
    // Self-hosted Umami — the same instance Eddy3D reports to, one "website" per Eddy3D-Dev
    // product. This id is MetaMAP's website on that instance.
    private const string UmamiEndpoint = "https://umami-theta-drab.vercel.app/api/send";

    private const string WebsiteId = "3425953c-0bc5-432e-9e0f-d3beba717eef";

    /// <summary>
    /// Environment variable that disables every send when set to <c>0</c>, <c>false</c>, <c>off</c>
    /// or <c>no</c>. Read once, when the plugin first touches analytics.
    /// </summary>
    public const string OptOutVariable = "METAMAP_ANALYTICS";

    private const string UnknownSlug = "unknown";

    // 1 hit + the identify payload's data properties (rhino_version, metamap_version).
    ...
    /// <summary>
    /// Interprets the <see cref="OptOutVariable"/> value: unset or anything else means enabled;
    /// <c>0</c>, <c>false</c>, <c>off</c>, <c>no</c> (any case) mean disabled.
    /// </summary>
    ...
    // ── What MetaMAP reports ────────────────────────────────────────────────────────────────

    /// <summary>
    /// The daily <c>/startup</c> page view — the denominator every tab share is read against.
    /// Called from <see cref="TabUsage.Install"/>, i.e. from the plugin's priority load.
    /// </summary>
    ...
    /// canvas, as a <c>/tab/&lt;slug&gt;</c> page view — once per machine per day. Anything that
    /// does not slug to a tab is ignored.
    /// </summary>
    ...
    /// <param name="action">What happened, e.g. <c>fetch</c>.</param>
    ...
    /// <summary>
    /// The URL segment for a ribbon panel: <c>"Building"</c> → <c>building</c>. The number
    /// prefix, if any, is dropped. Returns <c>unknown</c> for anything empty.
    /// </summary>
    ...
    /// <summary>
    /// Normalizes a value for use as a URL path segment (lowercase, runs of anything that is not
    /// a letter or digit become one hyphen). Keeps the reported paths low-cardinality so they
    /// stay groupable in Umami.
    /// </summary>
    ...
    /// <summary>
    /// Shared logic to send payloads to Umami. If eventName is null, it is treated as a Page View.
    /// </summary>
        ...
        bool budgetClaimed = false)
    {
        ...
        try
        {
            ...
            string metaMapVersion = GetPluginVersion();
            if (AnalyticsBudget.TryClaimIdentify(IdentifyCost, metaMapVersion))
            {
                await SendIdentifyAsync(url, eventName, title, userAgent, metaMapVersion);
            }
            ...
            {
                foreach (var prop in additionalData) dataObject[prop.Key] = prop.Value?.DeepClone();
            }
            ...
            var payloadData = new JsonObject
            {
                ...
                ["hostname"] = "metamap-plugin",
                ["language"] = "en-US",
                ["screen"] = "1920x1080",
                ["url"] = url,
                ["referrer"] = "https://archidynamics.com/",
                ["title"] = title
            };
    ...
    /// <summary>
    /// Sends the once-per-month <c>identify</c> payload describing this machine. The caller has
    /// already reserved <see cref="IdentifyCost"/> from the budget.
    /// </summary>
    private static async Task SendIdentifyAsync(
        string url, string? eventName, string title, string userAgent, string metaMapVersion)
    {
        try
        {
            var identifyData = new JsonObject
            {
                ...
                ["metamap_version"] = metaMapVersion
            };
    ...
    /// <c>data</c> because the header is part of the hit and therefore free, and because Umami
    /// derives its session id from exactly three inputs plus a salt.
    /// </summary>
    private static string BuildUserAgent(string visitorId)
    {
        return "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) " +
               $"Chrome/120.0.0.0 Safari/537.36 MetaMAP/1.0 (id:{visitorId})";
    }
    ...
    /// <summary>
    /// Gets the MetaMAP release version as a bare version string, preferring the SourceLink-free
    /// informational version so a pre-release suffix is reported as cut.
    /// </summary>
