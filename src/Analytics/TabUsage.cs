...
/// <summary>
/// Reports which MetaMAP <b>ribbon tabs</b> a machine uses, by watching every Grasshopper
...
/// <c>DocumentAdded</c> event and an explicit scan of already-open documents are needed to catch
/// objects that arrived from a loaded file. Call <see cref="Install"/> once from the plugin's
/// <c>GH_AssemblyPriority</c>; it is idempotent and never throws.
/// </remarks>
public static class TabUsage
{
    /// <summary>The Grasshopper category every MetaMAP component registers under.</summary>
    public const string Category = "MetaMAP";
    ...
    private static readonly ConditionalWeakTable<GH_Document, object> Hooked = new();

    /// <summary>
    /// Receives the raw <c>SubCategory</c> of every MetaMAP object observed. Defaults to
    ...
    /// <summary>
    /// Subscribes to the document server (once per process), scans the documents already open,
    /// and sends the daily <c>/startup</c> hit. Safe to call from the plugin's
    ...
    /// <summary>
    /// Hands the <c>SubCategory</c> of every MetaMAP object in <paramref name="objects"/> to
    ...
    public static void Observe(IEnumerable<IGH_DocumentObject>? objects)
    {
        ...
        {
            if (IsMetaMap(obj)) reporter(obj.SubCategory);
        }
    }

    /// <summary>
    /// A MetaMAP object is anything registered under <see cref="Category"/> with a ribbon panel.
    /// Params and components alike — the question is which TAB it came from, not what it is.
    /// </summary>
    public static bool IsMetaMap(IGH_DocumentObject? obj)
    {
